...
wro_camera.py — Vision server (Flask + HSV color detection)

Detects colors from the camera and publishes results as JSON at:
  GET http://localhost:5000/detections
  GET http://localhost:5000/video_feed  (live stream)

Official WRO 2026 colors (from rulebook pages 26-28):
...
# === CALIBRATED HSV RANGES FOR WRO — KERNEL ===
WRO_COLORS = {
    "obstaculo_rojo": {
        "ranges": [
            ([0,   150, 80],  [10,  255, 255]),   # Red wraps around 0/179 in HSV
...
def get_distance(area):
    """Estimate distance to detected object based on contour area."""
    if area > 50000:
        return "very_close"
    elif area > 20000:
        return "close"
...
def get_position(cx):
    """Return horizontal position of detected object (left/center/right)."""
    if cx < FRAME_W // 3:
        return "left"
    elif cx < 2 * FRAME_W // 3:
        return "center"
...
def process_frame(frame_bgr):
    """
    Process a camera frame and detect all WRO colors.
    Applies zone filter: orange and blue lines are only detected
    in the bottom half of the frame to avoid false detections from walls.
    """
    hsv = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2HSV)
    detections = {}

    # Draw zone dividers (left / center / right)
    zone_w = FRAME_W // 3
    cv2.line(frame_bgr, (zone_w, 0), (zone_w, FRAME_H), (50, 50, 50), 1)
    cv2.line(frame_bgr, (2*zone_w, 0), (2*zone_w, FRAME_H), (50, 50, 50), 1)
    cv2.putText(frame_bgr, "LEFT",   (10, 20),           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)
    cv2.putText(frame_bgr, "CENTER", (zone_w+10, 20),    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)
    cv2.putText(frame_bgr, "RIGHT",  (2*zone_w+10, 20),  cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)
    ...
    for name, config in WRO_COLORS.items():
        ...
        for cnt in contours:
            ...
            if area > config["min_area"]:
                ...
                # ZONE FILTER: orange and blue lines only in bottom half of frame
                # This prevents colored walls from triggering false line detections
    ...
    # Active detections panel (top right)
    y_text = 40
    cv2.putText(frame_bgr, "Active detections:", (FRAME_W-250, y_text),
...
def generate():
    """Generate MJPEG stream for the live video feed."""
    global output_frame
    while True:
        with lock:
...
def detections():
    """Return current color detections as JSON."""
    with lock:
